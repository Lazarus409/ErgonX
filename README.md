...
This build intentionally does not yet implement expenses, multi-document settlement allocation, bank reconciliation, customer/vendor tax profiles, VAT certificates, accounting compliance operations, payroll-to-accounting mapping, Recruitment, dashboards, Celery execution, advanced imports, salary bands, compensation review cycles, holiday-calendar configuration, or a PWA frontend. The Ghana minimum wage is versioned for compliance use but cannot yet be enforced against daily/casual compensation because the current ERD has no compensation pay-basis or paid-days field. Automated regulatory filing, direct GRA/SSNIT/NPRA integrations, full offline synchronization, offline payroll/payslip persistence, and web push remain outside the MVP boundary.
