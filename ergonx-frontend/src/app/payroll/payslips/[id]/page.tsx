...
export default function PayslipDetailPage() {
  ...
  if (!period) return <main className="p-4 md:p-6"><ErrorState title="Payslip details are incomplete" message="The payroll period was not returned by the server." onRetry={reload} /></main>;
  ...
  return <main className="space-y-6 p-4 md:p-6"><BackNavigation fallback="/payroll/payslips" label="Back to payslips" /><PageHeader title={`Payslip — ${period.name}`} description={`Generated ${formatDateTime(payslip.generated_at)} from a finalized payroll record.`} /><section className="mx-auto max-w-4xl rounded-xl border bg-white"><div className="border-b p-6"><div className="flex flex-col justify-between gap-5 sm:flex-row"><div><div className="flex items-center gap-2"><FileText size={20} /><h2 className="text-lg font-bold">{institution?.name ?? "Institution"}</h2></div><p className="mt-1 text-sm text-slate-500">Employee Payslip</p></div><div className="text-left text-sm sm:text-right"><p className="font-semibold">{period.name}</p><p className="text-slate-500">Pay date: {formatDate(period.pay_date)}</p></div></div></div><div className="grid gap-6 border-b p-6 sm:grid-cols-2"><Detail label="Employee" value={employee ? employeesApi.employeeDisplayName(employee) : EM_DASH} subvalue={employee?.employee_number} /><Detail label="Payroll period" value={`${formatDate(period.start_date)} – ${formatDate(period.end_date)}`} /></div><div className="grid gap-6 p-6 md:grid-cols-2"><ItemGroup title="Earnings" items={earnings} currency={payslip.payload.currency} totalLabel="Gross Pay" total={payslip.payload.gross_pay} /><ItemGroup title="Deductions" items={deductions} currency={payslip.payload.currency} totalLabel="Total Deductions" total={payslip.payload.total_deductions} /></div><div className="m-6 rounded-xl bg-slate-900 p-5 text-white"><div className="flex items-center justify-between"><span className="font-semibold">Net Pay</span><span className="text-2xl font-bold">{formatAmount(payslip.payload.net_pay, payslip.payload.currency)}</span></div></div><div className="border-t p-5"><div className="flex gap-3 text-sm text-slate-500"><Lock size={17} className="shrink-0" /><p>This payslip is generated from a finalized payroll record. Historical payroll data is immutable.</p></div></div></section>{!payslip.document_reference && <p className="mx-auto max-w-4xl text-center text-xs text-slate-500">No downloadable document has been attached to this payslip.</p>}</main>;
}
