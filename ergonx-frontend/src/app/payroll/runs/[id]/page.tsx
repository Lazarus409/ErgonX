  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border bg-surface p-5">
        ...
        {actions.length > 0 && <div className="mt-5 flex flex-wrap gap-2">{actions.map((action) => { const icon = action === "calculate" ? <Calculator className="h-4 w-4" /> : action === "submit" ? <Send className="h-4 w-4" /> : action === "approve" || action === "finalize" ? <CheckCircle2 className="h-4 w-4" /> : <XCircle className="h-4 w-4" />; return <button key={action} type="button" onClick={() => setPendingAction(action)} className={buttonClasses({ variant: "secondary", className: "inline-flex items-center gap-2" })}>{icon}{actionCopy[action].label}</button>; })}</div>}
