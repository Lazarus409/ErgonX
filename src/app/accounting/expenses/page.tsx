...
export default function ExpensesPage() {
  ...
  return <div className="space-y-6"><PageHeader title="Expenses" description="Create, approve and post institutional expenses." actions={allowed("expense.create") ? <button type="button" onClick={() => setCreating(true)} className={buttonClasses({ variant: "primary" })}><Plus className="h-4 w-4" />New Expense</button> : undefined} />{error && <ErrorState message={error} onRetry={reload} />}{actionError && <p className="rounded border border-danger/25 bg-danger-soft p-3 text-sm text-danger-ink">{actionError}</p>}{creating && <ExpenseForm onClose={() => setCreating(false)} onSaved={() => { setCreating(false); reload(); }} />}<section className="rounded-2xl border bg-surface p-5"><div className="flex flex-col gap-3 sm:flex-row"><div className="relative flex-1"><Search className="absolute left-3 top-3 h-4 w-4 text-ink-subtle" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search expense description..." className="w-full h-9 rounded-lg border pl-10 pr-4 text-sm" /></div><select value={status} onChange={(event) => setStatus(event.target.value)} className="rounded-xl border px-3 text-sm"><option value="ALL">All statuses</option>{["DRAFT", "PENDING", "APPROVED", "POSTED", "REJECTED"].map((value) => <option key={value}>{value}</option>)}</select></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[650px] text-sm"><thead><tr className="border-b text-left text-xs uppercase text-ink-muted"><th className="pb-3">Date</th><th className="pb-3">Description</th><th className="pb-3">Amount</th><th className="pb-3">Status</th><th className="pb-3">Actions</th></tr></thead><tbody>{shown.map((expense) => <tr key={expense.id} className="border-b last:border-0"><td className="py-4">{formatDate(expense.expense_date)}</td><td className="py-4 font-medium">{expense.description}</td><td className="py-4 font-semibold">{formatAmount(expense.amount, expense.currency)}</td><td className="py-4"><StatusBadge status={expense.status} /></td><td className="py-4"><div className="flex gap-2">{actionsFor(expense).map((action) => <button key={action} type="button" onClick={() => { setActionError(""); setPending({ expense, action }); }} className="rounded border px-3 py-2 text-xs font-semibold">{labels[action]}</button>)}</div></td></tr>)}</tbody></table>{loading && <p className="py-10 text-center text-sm text-ink-muted">Loading expenses...</p>}{!loading && !shown.length && <p className="py-10 text-center text-sm text-ink-muted">No expenses found.</p>}</div></section>{pending && <ConfirmDialog open title={copy[pending.action].title} description={copy[pending.action].description} confirmLabel={labels[pending.action]} destructive={copy[pending.action].destructive} loading={acting} onCancel={() => setPending(null)} onConfirm={() => void run()} />}</div>;
}
