  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border bg-surface p-5">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">Payroll Workflow</h2><p className="mt-1 text-sm text-ink-muted">Lifecycle state reported by the backend.</p></div><StatusBadge status={run.status} /></div>
        <div className="mt-5 grid gap-2 sm:grid-cols-6">{workflow.map((step, index) => <div key={step} className={`rounded-xl border p-3 text-center text-xs font-semibold ${index === statusIndex ? "bg-primary text-white" : index < statusIndex ? "bg-surface-sunken" : "bg-surface-muted"}`}>{step.replace("_", " ")}</div>)}</div>
        {actions.length > 0 && <div className="mt-5 flex flex-wrap gap-2">{actions.map((action) => { const icon = action === "calculate" ? <Calculator className="h-4 w-4" /> : action === "submit" ? <Send className="h-4 w-4" /> : action === "approve" || action === "finalize" ? <CheckCircle2 className="h-4 w-4" /> : <XCircle className="h-4 w-4" />; return <button key={action} type="button" onClick={() => setPendingAction(action)} className="inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-semibold hover:bg-surface-hover">{icon}{actionCopy[action].label}</button>; })}</div>}
        {run.status === "APPROVED" && !isReconciled && <p className="mt-4 text-sm text-warning-ink">Finalization remains unavailable until reconciliation completes without discrepancies.</p>}
      </section>

      <section className="rounded-2xl border bg-surface p-5">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">Reconciliation</h2><p className="mt-1 text-sm text-ink-muted">Derived totals are checked by the backend before approval and finalization.</p></div>{reconciliation && <StatusBadge status={isReconciled ? "RECONCILED" : "DISCREPANCY"} />}</div>
        {reconciliationError ? <p className="mt-4 text-sm text-warning-ink">Unable to load reconciliation: {reconciliationError}</p> : reconciliation ? <div className="mt-4 grid gap-3 sm:grid-cols-3"><Metric label="Records" value={String(reconciliation.record_count)} /><Metric label="Discrepancies" value={String(reconciliation.discrepancy_count)} /><Metric label="Result" value={isReconciled ? "Ready" : "Requires review"} /></div> : <p className="mt-4 text-sm text-ink-muted">Reconciliation is not yet available for this run.</p>}
      </section>

      <section className="rounded-2xl border bg-surface p-5"><div className="flex items-center justify-between"><div><h2 className="font-semibold">Employee Payroll Results</h2><p className="mt-1 text-sm text-ink-muted">Read-only records calculated by the backend.</p></div>{run.status === "FINALIZED" && <span className="inline-flex items-center gap-2 text-xs font-semibold"><Lock className="h-4 w-4" />Read-only</span>}</div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[700px] text-sm"><thead><tr className="border-b text-left text-xs uppercase text-ink-muted"><th className="pb-3">Employee</th><th className="pb-3">Gross</th><th className="pb-3">Deductions</th><th className="pb-3">Net Pay</th><th className="pb-3">Status</th></tr></thead><tbody>{records.map((record) => <tr key={record.id} className="border-b last:border-0"><td className="py-4 font-medium">{record.employee}</td><td className="py-4">{formatAmount(record.gross_pay, record.currency)}</td><td className="py-4">{formatAmount(record.total_deductions, record.currency)}</td><td className="py-4 font-semibold">{formatAmount(record.net_pay, record.currency)}</td><td className="py-4"><StatusBadge status={record.status} /></td></tr>)}</tbody></table>{records.length === 0 && <p className="py-8 text-center text-sm text-ink-muted">No payroll records have been calculated for this run.</p>}</div></section>

      <section className="rounded-2xl border bg-surface p-5"><h2 className="font-semibold">Payroll to Accounting</h2><p className="mt-1 text-sm text-ink-muted">{run.accounting_journal_entry ? "An accounting journal is linked to this payroll run." : run.status === "FINALIZED" && canGenerateJournal ? "Generate the idempotent draft journal, then submit, approve, and post it through Accounting." : "A draft accounting journal can only be generated from a finalized run by a user with Accounting journal-create access."}</p>{run.accounting_journal_entry && <Link href={`/accounting/journals/${run.accounting_journal_entry}`} className="mt-4 inline-flex text-sm font-semibold text-ink-strong hover:underline">Open accounting journal</Link>}{run.status === "FINALIZED" && !run.accounting_journal_entry && canGenerateJournal && <button type="button" onClick={() => setPendingAction("generateJournal")} className="mt-4 inline-flex rounded-xl border px-4 py-2.5 text-sm font-semibold hover:bg-surface-hover">Generate Accounting Journal</button>}</section>
...
function Metric({ label, value }: { label: string; value: string }) { return <div className="rounded-2xl border bg-surface p-4"><p className="text-xs text-ink-muted">{label}</p><p className="mt-2 break-words font-bold">{value}</p></div>; }
